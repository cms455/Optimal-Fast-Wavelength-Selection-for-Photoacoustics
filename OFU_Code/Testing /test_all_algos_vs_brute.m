...
random_diff_holder = zeros(num_repeats, k_items - 1);
...
for r = 1:num_repeats
    ...
    random_search_val_holder = zeros(1, k_items - 1);
    ...
    for k = 2:k_items
        ...
        random_times(k - 1) = toc;
        random_search_val_holder(k - 1) = min_inv_val;
        ...
        random_diff_holder(r, k - 1) = abs(brute_min_val - min_inv_val);
    ...
    plot(2:k_items, random_search_val_holder, '-o', 'DisplayName', 'Random Search', 'LineWidth', 2);
    ...
    plot(2:k_items, random_times, '-o', 'DisplayName', 'Random Search', 'LineWidth', 2);
...
for r = 1:num_repeats
    ...
    plot(2:k_items, random_diff_holder(r, :), 'Color', [1 0.5 0.5], 'LineWidth', 1);
...
mean_random_diff = mean(random_diff_holder, 1);
mean_nk_diff = mean(nk_diff_holder, 1);
plot(2:k_items, mean_luke_diff, 'b-', 'LineWidth', 3, 'DisplayName', 'Luke (mean)');
plot(2:k_items, mean_random_diff, 'r-', 'LineWidth', 3, 'DisplayName', 'Random Search (mean)');
...
save(fullfile(save_folder, file_name), ...
    ...
    'brute_search_val_holder', 'random_search_val_holder', 'luke_search_val_holder', 'nk_search_val_holder', ...
    'brute_times', 'random_times',  'luke_times', 'nk_times', ...
    'luke_diff_holder', 'random_diff_holder', 'nk_diff_holder');
